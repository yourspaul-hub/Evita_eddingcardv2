...
export function FooterSection() {
  return (
    <>
      {/* Smooth wave transition from cream RSVP into dark footer */}
      ...
      <section style={{ background: "linear-gradient(180deg,#4A1E2B,#2E1019)" }} className="py-20 px-6">
        <div className="max-w-lg mx-auto text-center">
          ...
          {/* Names — keeping Roshan & Evita order (user approved) */}
          <motion.h2 initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            transition={{ delay: 0.2 }}
            className="font-script mb-1" style={{ fontSize: "clamp(2.5rem,9vw,4rem)", color: "rgba(250,247,242,0.92)", lineHeight: 1.1 }}>
            {weddingConfig.couple.groomName}
          </motion.h2>
          <motion.span initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
            transition={{ delay: 0.3 }}
            className="font-cormorant italic text-2xl" style={{ color: "rgba(200,169,81,0.6)" }}>&amp;</motion.span>
          <motion.h2 initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            transition={{ delay: 0.35 }}
            className="font-script mt-1 mb-6" style={{ fontSize: "clamp(2.5rem,9vw,4rem)", color: "rgba(250,247,242,0.92)", lineHeight: 1.1 }}>
          ...
          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
            ...
            className="font-playfair mt-6 mb-1" style={{ color: "#C8A951", letterSpacing: "0.18em", fontSize: "1.05rem" }}>
          ...
          {/* Heart */}
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
            transition={{ delay: 0.7 }} className="mt-10 flex items-center justify-center gap-4">
